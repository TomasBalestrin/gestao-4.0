...
export function Sidebar({ role }: SidebarProps) {
  ...
  function renderItem({ href, label, icon: Icon }: NavItem) {
    ...
    return (
      <Link
        ...
        className={cn(
          "relative flex items-center gap-3 rounded-[8px] px-3 py-2 text-[13px] font-medium transition-colors duration-200 ease-out-expo",
          "focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background",
          active
            ? "bg-[var(--accent-soft)] text-primary font-medium"
      ...
      >
        {active && !collapsed && (
          <span
            aria-hidden
            className="absolute -left-3 top-1/2 h-[18px] w-[3px] -translate-y-1/2 rounded-pill bg-primary"
          />
        )}
  ...
  return (
    <aside
      className={cn(
        "hidden shrink-0 flex-col bg-[var(--surface-elevated)] border-r border-[color:var(--hairline)] lg:flex",
        collapsed ? "w-16" : "w-[220px]"
      )}
    >
      <div
        className={cn(
          "flex h-14 items-center gap-2 border-b border-[color:var(--hairline)] px-3",
          collapsed && "justify-center px-0"
        )}
      >
        {collapsed ? (
          ...
          <>
            <div className="flex min-w-0 flex-1 items-center gap-2.5">
              ...
              <span className="truncate text-[15px] font-semibold tracking-tighter text-foreground">
                Gestão 4.0
              </span>
            </div>
            <Button
            ...
            {isAdmin(role) && (
              <>
                <div className="px-3 pb-1 pt-5">
                  ...
                  {collapsed && <div className="h-px bg-[color:var(--hairline)]" />}
                </div>
...
function CloserNav({ collapsed, isActive, onWantExpand }: CloserNavProps) {
  ...
  function linkClass(active: boolean): string {
    return cn(
      "relative flex items-center gap-3 rounded-[8px] px-3 py-2 text-[13px] font-medium transition-colors duration-200 ease-out-expo focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2",
      active
            ...
            funis.map((f) => {
              ...
              return (
                <Link
                  ...
                  aria-current={active ? "page" : undefined}
                  className={cn(
                    "flex items-center gap-2 rounded-[8px] px-3 py-1.5 text-[13px] transition-colors duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
                    active
                      ? "bg-[var(--accent-soft)] text-primary font-medium"
                      : "text-text-secondary hover:bg-[var(--surface-glass)] hover:text-foreground"
                  )}
